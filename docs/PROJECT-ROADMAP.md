...
- Narrow v1 NVMe disk health is complete and dogfooded: Mounted Storage and `--diskcheck` can list physical NVMe drives, collect read-only SMART health, request explicit sudo fallback, save SQLite snapshots, and view readable saved reports in Walker.
...
8. Expand the current NVMe-only health check using [WOMBAT-WALKER-DISK-HEALTH-BRIEF.md](../briefs/WOMBAT-WALKER-DISK-HEALTH-BRIEF.md): SATA SSD/HDD, USB enclosure/pen-drive handling, comparative history, and optional confirmed self-tests.
