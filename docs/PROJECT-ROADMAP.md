...
- Narrow v1 NVMe disk health: Mounted Storage can list physical NVMe drives, collect read-only SMART health, request explicit sudo fallback, save SQLite snapshots, and view saved reports in Walker.
...
8. Expand the current NVMe-only health check using [WOMBAT-WALKER-DISK-HEALTH-BRIEF.md](../briefs/WOMBAT-WALKER-DISK-HEALTH-BRIEF.md): SATA SSD/HDD, USB enclosure/pen-drive handling, technical history, and optional confirmed self-tests.
